...
describe("RideRequestList", () => {
  ...
  if (req.body.username === 'Admin' && req.body.password === 'Admin@123RideShare') {
    req.reply((res) => {
    ...
    cy.intercept(
      ...
      cy.intercept(
        ...
        {
          fixture: "completed-rideRequests.json"
        }
      ...
      cy.intercept(
        ...
        {
          fixture: "rideRequests-fare.json"
        }
  ...
  it("handles error", () => {
    ...
    // cy.contains('There are no Ride Requests');
  ...
  it("handles search error", () => {
    ...
    cy.intercept(
      "GET",
      "https://rideshare-swdm.onrender.com/api/riderequests/filter?name=Eden%20HaipageNumber=1&pageSize=10",
      {
        fixture: "no-requests.json"
  ...
  it("checking above the table", () => {
    ...
    cy.get('tbody')
      ...
      // cy.get('.flex-col') error when fetching
  });
  ...
  it("displays the list of ride requests when data is available", () => {
    cy.fixture("no-requests.json").then((rideRequests) => {
      ...
      requests.forEach((rideRequest: any, index: number) => {
        ...
        cy.get(':nth-child(4) > .text-xl > .focus\\:outline-none').should("be.visible");
        // cy.get('.mx-auto') for spinner
      });
    });
